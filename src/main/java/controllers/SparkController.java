...
public class SparkController implements Serializable {
    ...
    public SparkController(SparkConf conf) {
        this.conf = conf;
    }
        ...
        JavaRDD<Sale> salesRDD = productsRDD.filter(new Function<Product, Boolean>() {
            ...
            public Iterable<Sale> call(Product product) throws Exception {
                ...
                for (int i = 0; i < 1000; i++) {
                    sales.add(new Sale(random.nextInt(), product.getId(), BigDecimal.valueOf(random.nextDouble())));
                }
                return sales;
            }
        });
    ...
    private void showResults(JavaSparkContext sc) {
        ...
        List<Tuple2<Product, com.google.common.base.Optional<Summary>>> results = productsRdd.leftOuterJoin(summariesRdd).values().toArray();
